...
namespace BlueOakBridge.EasyUnity
{
    public static class UnityEngineExtensions
    {
        ...
        public static Transform FindDeepInactiveChild(this Transform transform, string name)
        {
            for (int i = 0; i < transform.childCount; i++)
            {
                ...
                Transform t2 = FindDeepInactiveChild(transform.GetChild(i), name);
                if (t2) return t2;
            }
        ...
        {
            if (transform && index.Within(0, transform.childCount))
            {
