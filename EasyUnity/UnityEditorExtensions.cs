...
namespace BlueOakBridge.EasyUnity
{
    public static class UnityEditorExtensions
    {
        ...
        public static string GetOrCreateFolder(string parentFolder, string newFolderName)
        {
            if (AssetDatabase.IsValidFolder($"{parentFolder}/{newFolderName}"))
                return AssetDatabase.AssetPathToGUID($"{parentFolder}/{newFolderName}");
            else return AssetDatabase.CreateFolder(parentFolder, newFolderName);
        }
